...
def equal_line_notes(l,n=1,c=BLACK,size=DEFAULT_NOTE_SIZE):
    n = int(n)
    l=l.copy()
    p = l.get_center()[:2]
    ...
    def f(x):
        return W@x + p
    return VGroup(Line([*f([-0.37*0.5*(n-1)+0.37*i,-0.37*size]),0],[*f([-0.37*0.5*(n-1)+0.37*i,0.37*size]),0],color=c)for i in range(n))

def parallel_line_notes(l,n=1,c=BLACK,size=DEFAULT_NOTE_SIZE):
    n = int(n)
    l = l.copy()
    p = l.get_center()[:2]
    x1 = np.array([l.get_length(), 0])
    ...
    return VGroup(arr(-0.37*0.5*(n-1)+0.37*i) for i in range(n))
...
def equal_lines_notes_group(*lines,n=1,c=BLACK,size=DEFAULT_NOTE_SIZE):
...
def equal_angle_note(A,B,C,n=1,size=DEFAULT_NOTE_SIZE,color=BLACK):
    a = A.get_center()[:2]
    b = B.get_center()[:2]
    c = C.get_center()[:2]
    ...
    x = a-b
    y = c-b
    co = np.dot(x,y)/(np.linalg.norm(x)*np.linalg.norm(y))
    si = np.linalg.det(np.column_stack((x,y)))/(np.linalg.norm(x)*np.linalg.norm(y))
    t = math.acos(co)
    if si < 0:
        t = 2*PI-t
    ...
    if z[1]<0:
        ph = 2*PI-ph
    def ar(r):
        return Arc(arc_center=ex(b),radius=r,start_angle=ph,angle=t,color=color)
    return VGroup(*[ar(0.37*size+0.37*0.37*size*i)for i in range(n)])

def equal_angle_notes_solid(A,B,C,size=DEFAULT_NOTE_SIZE,color=BLACK,fill_opacity=0.37):
    a = A.get_center()[:2]
    b = B.get_center()[:2]
    c = C.get_center()[:2]
    ...
    return Sector(radius=size*0.37,arc_center=ex(b), start_angle=ph, angle=t, color=color,fill_opacity=0.37)

def equal_angle_notes_group(angles,n=1,size=DEFAULT_NOTE_SIZE,c=BLACK,is_change_color=False):
    return VGroup(*[equal_angle_note(*angles[i],n=n,size=size,color=c if not is_change_color and i < 2 else [LEAF_RED,LEAF_YELLOW][i]) for i in range(len(angles))])